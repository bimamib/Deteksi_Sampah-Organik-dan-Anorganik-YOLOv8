...
json_file_path = 'result/result_normal.json'
...
def change_directory_and_extension(image_path):
    label_path = image_path.replace(os.path.join('test/images'), os.path.join('test/labels'))
...
def draw_bounding_boxes(image_path, detection, labels, result_dir):
    ...
    cv2.putText(image, label_text, (xmin, ymin-10), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 255), 2)
    ...
    for label in labels:
        ...
        # Add class name for label in the bottom right corner
        label_text = f"Asli: {label['class_name']}"
        text_size = cv2.getTextSize(label_text, cv2.FONT_HERSHEY_SIMPLEX, 0.5, 2)[0]
        text_x = xmax_label - text_size[0]
        text_y = ymax_label + text_size[1] + 5
        cv2.putText(image, label_text, (text_x, text_y), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 0, 0), 2)
